...
import threading
from PIL import Image, ImageTk  # Asegúrate de instalar PIL o Pillow
...
def load_html():
    webview.create_window('D3.js Chart', './d3.html', width=800, height=600)
    webview.start()
...
def convert_plate_to_graphs_json(csv_path, plate, json_output_path):
    ...
    print(df.columns)
    matching_rows = df[df['num_plate'] == plate]  # Asegúrate de que 'num_plate' sea el nombre correcto de tu columna

    graphs = []

    for _, row in matching_rows.iterrows():
        route = row['route'].strip("[]").split(", ")  # Ajusta según sea necesario
        times = row['times'].strip("[]").split(", ")  # Ajusta según sea necesario
        nodes = set(route)
...
class ProfessionalTkinterGUI(tk.Tk):
    def __init__(self):
        ...
        self.processed_file_path = None  # Atributo para almacenar la ruta del archivo procesado
        self.distance_algorithm = tk.StringVar(value="levenshtein")  # Default value set to 'levenshtein'
    ...
    def create_widgets(self):
        ...
        button_width = 200  # Adjust this value as needed to control button width
        button_padx = (50, 50)  # Adjust padding to center buttons
        ...
        self.max_time_between_input.pack(side='left', fill='x', padx=(0, 10))  # The padding here should align with the label below
        self.max_time_between_input.insert(0, "1440")  # Default value
